...
final class WhisperSoundListeningService: ObservableObject, SoundListeningService {
    ...
    private let maxSeconds: Double = 12
    ...
    private let silenceSeconds: TimeInterval = 1.75
    ...
    private func appendBuffer(_ buffer: AVAudioPCMBuffer) {
        ...
        for i in 0..<frameLength {
            maxLevel = max(maxLevel, abs(pointer[i]))
        }

        let hasSpeech = maxLevel > speechThreshold
        ...
        if hasSpeech {
            ...
            if let startTime = silenceStartTime,
               Date().timeIntervalSince(startTime) >= silenceSeconds {

                print("Silence detected. Transcribing full phrase.")
    ...
    private func transcribeCurrentAudio() {
        ...
        guard samples.count > Int(inputSampleRate * 0.5) else {
