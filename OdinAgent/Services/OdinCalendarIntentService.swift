...
final class OdinCalendarIntentService {
    ...
    private let promptBuilder: any OdinCalendarIntentPromptBuilding

    init(
        ollamaService: any OdinLanguageModelServicing = OdinOllamaService(),
        promptBuilder: any OdinCalendarIntentPromptBuilding =
            OdinCalendarIntentPromptBuilder()
    ) {
    ...
    func parse(
        ...
        let prompt = promptBuilder.buildPrompt(command: command, now: Date())
